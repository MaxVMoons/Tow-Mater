...
class RaceConnection:
    ...
    def __init__(self, hostname):
        ...
        self.name = ""
        self.race_number = 0
        # This is where the UDP link to stream to will be stored.
        self.sendFeed = ""
    ...
    def racer_setup(self):
        ...
        self.sio.emit(
            "setup-racer", {"name": self.name, "number": self.team_number})
    ...
    def send_throttle(self, throttle):
        if self.connected:
            self.sio.emit("send-throttle",
                          {"teamNum": self.team_number, "throttle": throttle})
            ...
            if command.lower() == "n":
                ...
                while True:
                    if len(self.ip_address) != 0 and len(self.sendFeed) != 0:
                        # Wait until the server sends UDP link and stores IP address. Once loaded, break.
                        break
                break
